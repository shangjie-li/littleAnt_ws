...
bool PathTracking::init(ros::NodeHandle nh, ros::NodeHandle nh_private)
{
	...
	nh_private_.param<float>("max_match_distance", max_match_distance_, 5.0); // m
	nh_private_.param<float>("fd_speed_coefficient", fd_speed_coefficient_, 1.8);
	nh_private_.param<float>("fd_lateral_error_coefficient", fd_lateral_error_coefficient_, -1.0);
	nh_private_.param<float>("min_foresight_distance", min_foresight_distance_, 5.0); // m
	nh_private_.param<float>("max_side_acceleration", max_side_acceleration_, 1.0); // m/s2
...
void PathTracking::setExpectSpeed(float speed)
{
	expect_speed_ = speed;
}
...
void PathTracking::timer_callback(const ros::TimerEvent&)
{
	...
	// 航向偏差，自车左偏为负，自车右偏为正，限制在[-M_PI, M_PI]
	...
	if((cnt++) % 50 == 0)
	{
		ROS_INFO("cmd_v:%.2f\t true_v:%.2f\t offset:%.2f", cmd_.speed, vehicle.speed, offset_);
		ROS_INFO("yaw:%.2f\t dis_yaw.first:%.2f\t dis_yaw.second:%.2f", vehicle.pose.yaw, dis_yaw.first, dis_yaw.second);
		ROS_INFO("yaw_err:%.2f\t lat_err:%.2f\t foresight_distance:%.2f", yaw_err, lat_err, foresight_distance);
		ROS_INFO("nearest_idx:%lu\t target_idx:%lu\t final_index:%lu", nearest_idx, target_idx, global_path_.final_index);
		ROS_INFO("expect_v:%.2f\t curve_v:%.2f\t park_v:%.2f\t t_speed_mps:%.2f\t t_angle_deg:%.2f",
			expect_speed_, max_speed_by_curve, max_speed_by_park, t_speed_mps, t_angle_deg);
		ROS_INFO("pose:(%.2f, %.2f)\t nearest:(%.2f, %.2f)\t target:(%.2f, %.2f)",
			vehicle.pose.x, vehicle.pose.y, global_path_[nearest_idx].x, global_path_[nearest_idx].y, target_point.x, target_point.y);
		publishLocalPath();
	}
