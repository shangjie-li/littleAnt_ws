...
void AutoDrive::executeDriverlessCallback(const ant_actions::DoDriverlessTaskGoalConstPtr& goal)
{
	...
	while(ros::ok())
	{
		if(as_->isNewGoalAvailable())
		{
			...
			ant_actions::DoDriverlessTaskGoalConstPtr new_goal = as_->acceptNewGoal();
 ...
 *@param goal 目标信息
*/
bool AutoDrive::handleNewGoal(const ant_actions::DoDriverlessTaskGoalConstPtr& goal)
{
	...
	if(goal->task == goal->DRIVE_TASK)  //前进任务
    {
        ...
        if(goal->type == goal->POSE_TYPE) 
        {
			...
			as_->setSucceeded(ant_actions::DoDriverlessTaskResult(), 
        ...
        else if(goal->type == goal->PATH_TYPE)
        {
			if(!setDriveTaskPathPoints(goal))
			{
                ...
                as_->setSucceeded(ant_actions::DoDriverlessTaskResult(), "Aborting on drive task, because The target path is invalid!");
				return false;
			}
        }
        //指定路径文件
        else if(goal->type == goal->FILE_TYPE)
        {
            if(!loadDriveTaskFile(goal->roadnet_file))
            {
                ...
                ant_actions::DoDriverlessTaskResult res;
                res.success = false;
        ...
        else
        {
			...
			as_->setAborted(ant_actions::DoDriverlessTaskResult(), "Aborting on unknown goal type! ");
            return false;
        }
        //切换系统状态为: 切换到前进
	...
	else
	{
		...
		as_->setAborted(ant_actions::DoDriverlessTaskResult(), "Aborting on unknown task! ");
		return false;
	}
	ROS_ERROR("[%s] Unknown error type!", __NAME__);
	as_->setAborted(ant_actions::DoDriverlessTaskResult(), "Aborting on unknown error! ");
	return false;
}
...
void AutoDrive::doDriveWork()
{
	...
	while(ros::ok() && system_state_ != State_Stop && avoider_.isRunning())
	{
		...
		{
			ant_actions::DoDriverlessTaskFeedback feedback;
	...
	if(as_->isActive())
	{
		as_->setSucceeded(ant_actions::DoDriverlessTaskResult(), "drive work  completed");
	}
	task_running_ = false;
...
ant_msgs::ControlCmd2 AutoDrive::driveDecisionMaking()
{
	...
	if(tracker_cmd_.turnLight == 0)
	{
	    ...
	    float offset_temp = avoider_.getOffset();
